...
MongoClient.connect(url, function (err, db) {
    ...
    dbo.listCollections().toArray(function (err, items) {
        ...
        var i;
        var staticFileFlag = false;

        var formReplacePlaceHolder = new Map();
        for (i = 0; i < items.length; i++) {
            console.log("in side for loop");
            if (items[i].name == 'staticFileTable') {
                staticFileFlag = true;
                console.log("found staticFileFlag");

                // retrieve and set into staticFile map ??? 
                dbo.collection("staticFileTable").find({}).toArray(function (err, result) {

                    for (var i = 0; i < result.length; i++) {
                        staticFiles.set(result[i]["fileName"], result[i]["fileType"]);
                    }
        ...
        if (staticFileFlag == false) {

            // create the password table at here
            console.log("in here")
            dbo.createCollection("staticFileTable", function (err, res) {
                if (err) throw err;
                console.log("staticFileTable Collection first created!");
            });
        }
    });
    //------------------------------------------------------------------
        ...
        socket.on("data", function (data) {
            ...
            if (clients.includes(clientID) == true) {
                ...
                if (bufferRead == false) {
                    ...
                    if (opcode == 8) {
                        console.log("193 close websocket");

                        for (const [key, value] of username_socket.entries()) {


                            if (socket == value) {
                                username_socket.delete(key);
                                var onlineusernameJSON = JSON.stringify({ 'disconnectUsername': key });
                        ...
                        /* Here is receive websocket frame from client*/
                            ...
                            console.log("296 commentAfterReplace: " + commentAfterReplace);
                            var myobj = JSON.parse(commentAfterReplace);

                            //hw6
                            //3b set flag chating to true, and store the username of clicked button in chatingPerson
                        ...
                        // for actual extend payload length is X>=126 X<65,536
                            ...
                            if (opcode == 0 && finBit == 128 && useCumulateFrameFlag == true) {
                                ...
                                var profileUsername = '';
                                var imageStorePath = '';
                                if (senderNameinStr == 'Confirm') {

                                    for (const [key, value] of username_socket.entries()) {
                                        if (value == socket) {
                                            console.log("428key" + key);

                                            profileUsername = key;
                                        }

                                    }
                                    //4d once receive update message, update image store in db
                                    imageStorePath = 'image/' + profileUsername + '.jpg';

                                    var userProfileKeyValue = '{ "username":' + '"' + profileUsername + '"' + ', ' + '"path":' + '"' + imageStorePath + '"' + '}';
                                    var myprofileobj = JSON.parse(userProfileKeyValue);
                                    console.log("442" + myprofileobj);
                                    dbo.collection("userProfilePath").findOne({ username: profileUsername }, { _id: 0 }, function (err, result) {
                                        // if this is the new user then insert 
                                        if (err) throw err;
                                        console.log("445" + result);

                                        // update user's token in the table just changed for hw5
                                        var myquery = { username: profileUsername };
                                        var newvalues = { $set: myprofileobj };
                                        dbo.collection("userProfilePath").updateOne(myquery, newvalues, function (err, res) {
                                            if (err) {
                                                console.log("536")
                                                throw err;

                                            }
                                            console.log("459Profile update");
                                        });
                                ...
                                fs.writeFile(imageStorePath, buf1, 'binary', function (err) {
                                    ...
                                    if (senderNameinStr == 'Confirm') {
                                        ...
                                        sendContent = JSON.stringify({ 'onlineusername': profileUsername, 'ProfileImagePath': 'image/' + profileUsername + '.jpg' });
                                    }
                                    else {
                                        sendContent = JSON.stringify({ 'SenderName': senderNameinStr, 'ImagePath': imagePathInBi });
                                    }
                                    ...
                                    if (JSONsize < 126) {
                                        ...
                                        //3c once chating is true, only send message to self and the person he want to DM
                                        if (chating == true && (sendContent.includes("SenderName"))) {
                                            username_socket.get(chatingPerson).write(newbuff);
                                            socket.write(newbuff);
                                        }
                                        else {
                                        //4d send update image path to every user
                            ...
                            // receive small image
                                ...
                                if (senderNameinStr == 'Confirm') {
                                    ...
                                    // update profileImagePath in userProfile table  ??
                                    var userProfileKeyValue = '{ "username":' + '"' + profileUsername + '"' + ', ' + '"path":' + '"' + imageStorePath + '"' + '}';
                                    var myprofileobj = JSON.parse(userProfileKeyValue);
                                    console.log("442" + myprofileobj);
                                    dbo.collection("userProfilePath").findOne({ username: profileUsername }, { _id: 0 }, function (err, result) {
            ...
            // if not websocket then handle as http POST GET request
                ...
                // parse this as request header and some part of content body part
                    ...
                    var onlyCharArray = '';
                    if (requestPath.substring(7, 10) != 'new') {
                        for (var i = 0; i < requestPath.length; i++) {
                            if (requestPath[i] >= "0" && requestPath[i] <= "9") {
                                //str[i] >= "0" && str[i] <= "9"
                            } else {
                                onlyCharArray += requestPath[i];
                            }

                        }
                        requestPath = onlyCharArray;
                    }

                    console.log("1538 image path (requestpath) " + requestPath)
                    ...
                    if (requestType === "POST" && requestPath === "/registration") {
                        ...
                        if (checkFlag == false) {
                            ...
                            var defaultPath = 'image/default.jpg';
                            var userProfileKeyValue = '{ "username":' + '"' + namePassArray[0] + '"' + ', ' + '"path":' + '"' + defaultPath + '"' + '}';
                            //var myobj = JSON.parse({ "name": commentorNameInString, "password": hash });
                            var myProfileObj = JSON.parse(userProfileKeyValue);
                            ...
                            dbo.listCollections().toArray(function (err, items) {
                                ...
                                for (i = 0; i < items.length; i++) {
                                    ...
                                    if (items[i].name == 'userProfilePath') {
                                        profileTableExist = true;
                                        console.log("found profileTableExist table, then insert");

                                        // if username exist then prevent insert
                                        // find the username exist or not first
                                        
                                        //4a once register, store a default path in db
                                        dbo.collection("userProfilePath").findOne({ name: namePassArray[0] }, { _id: 0 }, function (err, result) {
                                            if (err) throw err;

                                            if (result == null) {
                                                registerFlag = true;
                                                console.log("1553 new user default profile insert");
                                                //insert into password colloection
                                                //var myobj = JSON.parse(passwordKeyValue);
                                                dbo.collection("userProfilePath").insertOne(myProfileObj, function (err, res) {
                                                    if (err) throw err;
                                                    console.log("new user default profile insert inserted: " + res.insertedCount);
                                                    //registerFlag = true;// if you put here it will wait long time 
                                                });
                                            }
                                        });
                                ...
                                if (profileTableExist == false) {
                                    // create the password table at here
                                    console.log("in 1598")
                                    dbo.createCollection("userProfilePath", function (err, res) {
                                        if (err) throw err;
                                        console.log("userProfilePath Collection first created!");
                                        //db.close();
                            ...
                            // inset 
                            var fileTypeKeyValue = '{ "fileName":' + '"' + "/image/" + namePassArray[0] + ".jpg" + '"' + ', ' + '"fileType":' + '"image/jpeg"' + '}';
                            var staticTableobj = JSON.parse(fileTypeKeyValue);
                            dbo.collection("staticFileTable").insertOne(staticTableobj, function (err, res) {
                                if (err) throw err;
                                console.log("new static file inserted: " + res.insertedCount);
                                //registerFlag = true;// if you put here it will wait long time 
                            });
                    ...
                    // hw2_object 1  & hw 4 & hw5 root
                        ...
                        if (n < lines.length) {
                            ...
                            for (var k = 0; k < cookieLinePart.length; k++) {
                                ...
                                if (keyOfCookie == ' Authentication' || keyOfCookie == 'Authentication') {
                                    ...
                                    dbo.collection("userToken").findOne({ token: comparedToken }, { _id: 0 }, function (err, result) {
                                        ...
                                        if (result == null) {
                                            ...
                                            //
                                            //profileImageUpload  websocket form
                                            var uploadProfilePart = '<div> <label for="Profile_uploads">Upload Profile Image (JPG file) </label>';
                                            uploadProfilePart += ' <input type="file" id="profileImageID" name="profileImage" accept="image/jpeg">';
                                            uploadProfilePart += '<br /><label for="Sender-Name">Type Confirm to agree to update: </label>';
                                            uploadProfilePart += '<input id="profileUsername" type="text" name="Confirm">';
                                            uploadProfilePart += '<br /></div><button onclick="updateProfileImage();">Update Profile Image</button>';
                                            console.log("firstTimeVistFlag: " + firstTimeVistFlag);
                                            console.log("authenticatedFlag: " + authenticatedFlag);

                                            var profileImageRetrievePath = '';

                                            if (authenticatedFlag == true) {

                                                dbo.collection("userProfilePath").findOne({ username: usernameInToken }, { _id: 0 }, function (err, result) {
                                                    if (err) throw err;

                                                    if (result != null) {

                                                        profileImageRetrievePath = result.path;
                                                    }
                                                    console.log("double true here");
                                                    let formReplacePlaceHolder = new Map([
                                                        ["title", "You Logged In as " + result.username],
                                                        ["profileImage", 'Profile Image is:  <div id=' + justSignInUser + '>  <img src="' + profileImageRetrievePath + '" "width="150" height="150" >  </div>'],
                                                        ["uploadProfile", uploadProfilePart],
                        ...
                        dbo.collection("password").findOne({ name: namePassArray[0] }, { _id: 0 }, function (err, result) {
                            ...
                            if (!bcrypt.compareSync(namePassArray[1], result.password)) {
                                ...
                                */
                                ...
                                // retreive path from table
                                
                                //4b send login user with its image path that store in db
                                dbo.collection("userProfilePath").findOne({ username: namePassArray[0] }, { _id: 0 }, function (err, result) {
                                    if (err) throw err;
                    ...
                    //----------------------------
                        ...
                        */
                        ...
                        // here we do not insert any record, just retrieve comment out for project, no need retrieve for DM
                        /*
                        ...
                        //2b add username and its socket into a global variable call username_socket
                        username_socket.set(justSignInUser, socket);

                        dbo.collection("userProfilePath").find({}).toArray(function (err, result) {
                            console.log("1626");
                            if (err) throw err;
                            ...
                            for (const [key, value] of username_socket.entries()) {
                                ...
                                for (var i = 0; i < result.length; i++) {
                                    //Convert a JavaScript object into a string
                                    console.log("997result[i]" + result[i]["username"]);
                                    console.log("998result[i]" + result[i]["path"]);
                                    var jsonToString = JSON.stringify(result[i]);
                                    console.log("999" + jsonToString);
                                    if (result[i]["username"] == key) {
                                        profileImageRetrievePath = result[i]["path"];
                                    }

                                }


                                //2d json will include username and its profile image
                                var onlineusernameJSON = JSON.stringify({ 'onlineusername': key, "ProfileImagePath": profileImageRetrievePath });
                                // for profile image
                                // var profileImagePath = 'image/' + key.toString() + '.jpg';
                                //var profileContent = JSON.stringify({ 'onlineusername': key, 'profileImagePath': profileImagePath });

                                console.log("1833: " + onlineusernameJSON);

                                const JSONsize = Buffer.byteLength(onlineusernameJSON);
                                //const JSONsize = Buffer.byteLength(profileContent);
                                ...
                                var buf1 = Buffer.from(sendFrameArray);
                                var buf2 = Buffer.from(onlineusernameJSON);
                                //var buf2 = Buffer.from(profileContent);

                                var list = [buf1, buf2];
                                var buff = Buffer.concat(list);
                                //2c once the user is added into username_websocket, 
                                //send all username that were appended into username_socket
                                //to every user
                                for (const [key1, value2] of username_socket.entries()) {
                                    if (key1 != key) {
                                        console.log("value2" + value2);
                                        if (key1 == 'jessi')
                                            console.log("2182 jessi: " + onlineusernameJSON)
                                        value2.write(buff);
                                    }
